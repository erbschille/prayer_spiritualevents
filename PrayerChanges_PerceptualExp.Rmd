---
title: "The Effects of Prayer on Spiritual Presence Events"
...
```{r}
# Prepare the subset for charismatic participants ahead
charismatic_subset <- d_e_m_sum_all %>% 
  filter(population == "charismatic")

ggplot() +
  # Jitter for all participants colored by country
  geom_jitter(data = d_e_m_sum_all, aes(x = prayminday_imp0, y = spev_score, color = population), 
              height = 0, width = 0.2, alpha = 0.8, size = 1) +
  # Regression line with confidence interval for all participants
  geom_smooth(data = d_e_m_sum_all, aes(x = prayminday_imp0, y = spev_score, group = population, color = population), 
              method = "lm", se = TRUE, linetype = "solid", alpha = 0.2) +
  scale_x_continuous(breaks = 0:4, labels = levels(d_e_m_sum_all$prayminday_imp0_cat)) +
  theme(legend.position = "top",
...
ggplot(aes(x = prayminday_imp0, y = spev_score)) +
...
```{r}
m.epi.b.1p <- lmer(
  spev_score ~ 1 + scale(prayminday_imp0, scale = F) 
...
m.epi.b.2p <- lmer(
  spev_score ~ 1 + scale(prayminday_imp0, scale = F) 
  + (1|country),
  data = d_e_m_sum_all %>% filter(population == "charismatic"),
  contrasts = list(
    country = contr_effect_fun(
      d_e_m_sum_all %>% filter(country != "China"), 
      "country", 
      "US")))
```
...
d_i_m %>%
  ...
  geom_jitter(width = 0.2, height = 0.1, size = 0.7, alpha = 0.4) +
...
d_e_m %>% 
  ...
  left_join(d_e_m %>% distinct(question, category)) %>%
  mutate(category = if_else(is.na(category), "(other)", category)) %>%
  group_by(category) %>%
  mutate(MEAN_cor = mean(cor_prayer, na.rm = T)) %>%
  ungroup() %>%
...
#prayminday_imp0
m.epi.3b.1p.1 <- lmer(
  spev_score ~ 1 + category * scale(prayminday_imp0, scale = F)
...
```{r}
#prayminday_imp0 -- not enough variability in pray at all, basically all charismatics pray
contrasts(d_e_m$category) <- contr_effect_fun(d_e_m, "category", "weird") # drop weird

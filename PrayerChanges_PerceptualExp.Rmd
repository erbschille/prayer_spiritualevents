---
title: "Prayer Changes Perceptual and Other Experiences--y/n"
author: "Eleanor Schille-Hudson & Kara Weisman"
date: "2024-03-05"
...
Worth noting that all responses for study 1 are yes/no. No frequency
...
```{r}
m.int.a.1 <- lmer(
  spev_score ~ 1 + scale(prayfreqmin, scale = F) 
...
```
### all participants--controlling for religiosity
...
### Non-Christian participants, grouped by country 

```{r}
d_i_m_sum_all %>%
  filter(religion == "local") %>%
  filter(country != "US") %>%
  filter(!(country == "Vanuatu" & site != "rural")) %>%
  filter(!is.na(prayfreqmin)) %>%
ggplot(aes(x = prayfreqmin, y = spev_score)) +
  geom_jitter(aes(color = country), 
              height = 0, width = 0.2, alpha = 0.8, size = 1) +
  geom_smooth(aes(group = country, color = country), 
              method = "lm", se = TRUE, alpha = 0.1) +
  facet_wrap(~ country, ncol = 5) +  
  scale_x_continuous(breaks = 1:4, labels = levels(d_i_m_sum_all$prayfreqmin_cat)) +
  scale_color_brewer(palette = "Set2") +
  labs(title = "Study 1, NON Christians",
       x = "'How many minutes each day do you [pray] on average throughout the week?'",
       y = "Spiritual Events Score",
       color = "Country") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), 
...
```{r}
m.epi.b.1p <- lmer(
  spev_score_f_scaled ~ 1 + scale(prayminday_imp0, scale = F) 
  + (1|country/population),
  data = d_e_m_f_sum_all)
```

```{r}
parameters(m.epi.b.1p)
```

### Charismatic Christian participants only 

**An informal summary (KW)**: Yet again, all of this held true considering only charismatic samples, and this is robust to many analytic decisions, including modeling responses at the item level vs. modeling average scores on the Spiritual Events scale and considering sample characteristics as fixed or random effects. 
...
m.epi.rel.all.1 <- lmer(
  spev_score_f_scaled ~ 1 + scale(prayminday_imp0, scale = F) + scale(howr_num, scale = F)
  + (1|country/population),
  data = d_e_m_f_sum_all)
```
...
### Charismatic Christians controlling for religiosity
...
### General population participants, grouped by country 
...
ggplot(aes(x = prayminday_imp0, y = spev_score_f)) +
  ...
  scale_x_continuous(breaks = 0:4, labels = levels(d_e_m_f_sum_all$prayminday_imp0_cat)) +
  scale_color_brewer(palette = "Set2") +
  labs(title = "Study 2, General Population only",
       x = "'Do you [pray] regularly? On an average day, how many minutes do you [pray]?'\nNOTE: Participants who explicitly reported never praying were assigned 'not at all' (0)",
       y = "Spiritual Events Score",
       color = "Country") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), 
...
**An informal summary (KW)**: above and beyond differences across denominations, the more frequently a person reported praying, the more spiritual events they reported having experienced. This is robust to many analytic decisions, including [NOT YET CONFIRMED: modeling responses at the item level vs. modeling average scores on the Spiritual Events scale and] considering sample characteristics as fixed vs. random effects. 
...
d_p_m_sum <- d_p_m_sum %>%
  mutate(group = case_when(
    ...
    religion_denom == "Christian" ~ "All Christian",
...
ggplot(d_p_m_sum, aes(x = pray_freq_PEW_num, y = spev_score, color = group)) +
  ...
  geom_smooth(data = d_p_m_sum %>% filter(group == "All Christian"), aes(group = group),  method = "lm", alpha = 0.1) +
  ...
  labs(title = "Study 3, Comparing all, Christian, and evangelical participants",
...
```{r}
m.pro.a.1 <- lmer(
  spev_score_scaled ~ 1 + scale(pray_freq_PEW_num, scale = F) 
...
```{r}
parameters(m.pro.a.1)
```

### All Christian participants

**An informal summary (KW)**: The previous observation also holds true for Christian participants considered alone: above and beyond differences across denominations, the more frequently a person reported praying, the more spiritual events they reported having experienced. Again, this is robust to many analytic decisions, considering sample characteristics as fixed vs. random effects. 
...
```{r}
parameters(m.pro.a.2)
```

### Only evangelical Christian participants

**An informal summary (KW)**: The previous observation also holds true for Evangelical participants considered alone: the more frequently a person reported praying, the more spiritual events they reported having experienced.

```{r}
m.pro.a.3 <- lm(
  spev_score ~ 1 + scale(pray_freq_PEW_num, scale = F),
  data = d_p_m_sum %>% filter(grepl("Evang", religion_cat)))
```

```{r}
parameters(m.pro.a.3)
```
### all participants controlling for religiosity

```{r}
m.pro.rel.all.1 <- lmer(
  spev_score_scaled ~ 1 + scale(pray_freq_PEW_num, scale = F) + scale(demo_religimp_num, scale = F)
  + (1|religion_denom), # fully nested
...
### christian participants controlling for religiosity
...
### evangelical participants controlling for religiosity

```{r}
m.pro.rel.ev.1 <- lm(
  spev_score ~ 1 + scale(pray_freq_PEW_num, scale = F) + scale(demo_religimp_num, scale = F),
  data = d_p_m_sum %>% filter(grepl("Evang", religion_cat)) %>%
    left_join(d_p_demo %>% select(ResponseId, demo_religimp_num)))
```

```{r}
parameters(m.pro.rel.ev.1)
```

### Non-religious participants only 

```{r, fig.width = 15}
d_p_m_sum %>%
  filter(religion_denom == "Non-religious") %>%
  #filter(religion_denom != "Christian") %>%
  #filter(religion_denom == "New Age / Spiritual") %>% 
  filter(religion_cat != "NA") %>% 
ggplot(aes(x = pray_freq_PEW_num, y = spev_score)) +
   geom_jitter(aes(color = religion_cat), height = 0, width = 0.2, alpha = 0.7, size = 0.1) +
  geom_smooth(aes(group = religion_cat, color = religion_cat), 
              method = "lm", se = TRUE, alpha = 0.1) +
  facet_wrap(~ religion_cat, ncol = 7) +  
  scale_x_continuous(breaks = 1:7, labels = levels(d_p_m_sum$pray_freq_PEW)) +
  scale_y_continuous(breaks = 0:4, limits = c(0, 4)) +
  scale_color_brewer(palette = "Set2") +
  labs(title = "Study 3, Non-Religious participants only",
       x = "'Outside of attending religious services, how often do you pray?'",
       y = "Spiritual Events score (mean response)",
       color = "Group") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), 
...
```{r}
m.int.3a.1.1 <- lmer(
  spev_score ~ 1 + category * scale(prayfreqmin, scale = F)
  + (1|country/site/religion), # fully nested
  data = d_i_m_sum)
```
...
m.int.3a.2.2 <- lmer(
  ...
  + (1|country/site), # subject nested within site nested within country, no religion bc all christian
...
m.epi.3b.1p.1 <- lmer(
  spev_score_f_scaled ~ 1 + category * scale(prayminday_imp0, scale = F)
  + (1|country/population), # fully nested
  data = d_e_m_f_sum)
```

```{r}
parameters(m.epi.3b.1p.1)
```
...
```{r}
parameters(m.epi.3b.2p.1)
```

### Mixed effects models controlling for religiosity and with random effects by question
...
m.epi.rel.all <- lmer(
  spev_score_f_scaled ~ 1 + category * scale(prayminday_imp0, scale = F) + scale(howr_num, scale = F)
  + (1|country/population), # fully nested
  data = d_e_m_f_sum)
```
...
m.pro.3a.1.1 <- lmer(
  score ~ 1 + category * scale(pray_freq_PEW_num, scale = F)
  # + (1|religion_denom/ResponseId), # fully nested, takes forever to run...
  + (1|ResponseId), # no religion
  # control = lmerControl(optimizer = "bobyqa"),
  data = d_p_m %>% 
    group_by(ResponseId, religion_denom, pray_freq_PEW_num, category) %>%
    summarise(score = mean(response, na.rm = T)/4),
  contrasts = list(category = contr_effect_fun(d_p_m, "category", "presence")))
```

```{r}
parameters(m.pro.3a.1.1)
```
...
```{r}
parameters(m.pro.3a.1.3)
```

### Mixed effects models controlling for religiosity 
...
# category-level "scores"
m.pro.rel.all <- lmer(
  score ~ 1 + category * scale(pray_freq_PEW_num, scale = F) + scale(demo_religimp_num, scale = F)
  + (1|ResponseId), # no religion
  # control = lmerControl(optimizer = "bobyqa"),
  data = d_p_m %>% 
    filter(religion_denom == "Christian") %>%
